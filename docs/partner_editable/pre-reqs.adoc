...
+
NOTE: Consider choosing a Region where you want monitor Amazon EC2 intance network traffic using VPC Traffic Mirroring.
...
You might want to use single target for multiple sources. In doing so target can become resource constrained. You need to benchmark your performance to appropriately size your EC2 instance running Suricata.

You can also horizontally scale EC2 instances by front ending them with a network load balancer (NLB) and use NLB as a target instead. 
